class Game {
  ...
  showFuelBar() {
    push();
    image(fuelImage, width / 2 - 130, height - player.positionY - 2250, 20, 20);
    fill("white");
  ...
  handlePlayerControls() {
    if (keyIsDown(UP_ARROW) && this.carbroken == false) {
    ...
    if (keyIsDown(LEFT_ARROW) && player.positionX > width / 3 - 50 && this.carbroken == false) {
      this.leftKeyActive = true;
      player.positionX -= 5;
      player.update();
    }

    if (keyIsDown(RIGHT_ARROW) && player.positionX < width / 2 + 300 && this.carbroken == false) {
      this.leftKeyActive = false;
      player.positionX += 5;
      player.update();
    }
